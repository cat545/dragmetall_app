...
class FavoritesPage extends StatefulWidget {
  bool? fromSingle;
  FavoritesPage({Key? key, this.fromSingle}) : super(key: key);
...
class _FavoritesPageState extends State<FavoritesPage> with TickerProviderStateMixin{
  ...
  @override
  void initState() {
    requestNewScreen("<favorites page>");
    startElementsList();
  ...
  getElementsList() {
    ...
    elementsList = [];
    elements = DBHelper().db_getFavorites();
    elements.then((data) {
      elementsList = data;
      isLoading = false;
    });
      ...
      child: Row(
        ...
        children: [
          ...
          Row(
            children: [
              IconButton(
                icon: Icon(Icons.stars, color: Colors.amberAccent[400], size: 28),
                onPressed: () {},
              ),
              IconButton(
  ...
  Widget element(elementPage) {
    return ListTile(
      onTap: () {
        elementPage.way = elementPage.way.toString().replaceAll("^${elementPage.name}", "");
        if(widget.fromSingle != null && widget.fromSingle!){
          Navigator.of(context).pop({"way": elementPage.way, "titlePage": elementPage.name, "singleElement": elementPage});
        }else
        {
          Navigator.of(context).pop();
          Navigator.push(context, MaterialPageRoute(builder: (context) => SinglePage(way: elementPage.way, titlePage: elementPage.name, singleElement: elementPage )));
        }
        

        //Navigator.push(context, MaterialPageRoute(builder: (context) => SinglePage(way: elementPage.way, titlePage: elementPage.name, singleElement: elementPage )));
      },
      trailing: const Icon(Icons.equalizer, size: 25, color: Colors.black54),
      contentPadding: const EdgeInsets.symmetric(horizontal: 10),
      subtitle: subtitle(elementPage),
      title: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          SizedBox(
        ...
        child: Row(
          ...
          children: [
            Expanded(
              child: Row(
                children: [
                  ...
                  const Icon(Icons.star_rate, color: Colors.white38),
                  const SizedBox(width: 8),
                  Expanded(
                    child: Text(
                        "Избранное",
            ...
            IconButton(
              ...
              onPressed: () async {
                requestNewScreen("<favorites page> <button> delete");
                //<favorites page>
                await showEraseFrom(context);
                getElementsList();
                setState(() {});
              })
          ],
  ...
  Future<void> showEraseFrom(BuildContext context) async {
                ...
                Row(
                  ...
                  children: [
                    GestureDetector(
                      ...
                      onTap: () async{
                        requestNewScreen("<favorites page> <button> confirm yes");
                        Navigator.of(context).pop();
                        await DBHelper().db_delAllFavorites();
                      }),
                      GestureDetector(
                      child: Padding(
                              padding: const EdgeInsets.all(8.0),
                      ...
                      onTap: () async{
                        requestNewScreen("<favorites page> <button> confirm no");
                        Navigator.of(context).pop();
                        await DBHelper().db_delAllFavorites();
                      }),
                  ],
